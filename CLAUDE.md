...
- **2026-05-12 (Supabase cleanup — KOMPLETIRANO u istom danu)**: Migracije `legacy_rename_unused_tables` + `drop_legacy_unused_tables`. **10 neiskorišćenih tabela obrisano**: `users`, `roles`, `user_roles`, `address_units`, `survey_statuses`, `survey_records`, `resident_entries`, `address_change_requests`, `audit_logs`, `editor_streets` — sve su imale 0 redova, app ih nije dirao (grep `index.html` + Edge Function + DB funkcije + `pg_depend` graf potvrdili). Pristup: prvo rename u `_legacy_*`, korisnik uradio smoke test live app-a (login, evidencija, status promena, pretraga, mapa, admin panel — sve radi), pa drop CASCADE u istoj sesiji. Pred rename napravljen svež pg_dump backup kao rollback tačka. **Advisor: performance 25 → 11, security 15 → 14**.

- **2026-05-12 (Admin UI za assignment log + drop nepotrebnih indeksa)**:
  - **UI**: Nova admin sekcija „📜 Istorija dodela ulica" u admin panel-u (ispod „Templates napomena", iznad „Import"). Funkcija `renderAssignmentHistory()` povlači poslednjih 100 unosa iz `assignment_log` sa embedded JOIN na `profiles` (user + changed_by) i `streets`. 4 tipa akcije sa ikonom/bojom: ➕ Dodeljen (zeleno), ⏸️ Deaktiviran (žuto), ▶️ Reaktiviran (plavo), 🗑️ Uklonjen (crveno). Učitava se na otvaranje admin panel-a i na „Osveži korisnike" dugme.
  - **Migracija `drop_unused_indexes`**: obrisano 8 indeksa koji su flag-ovani kao unused i nisu ni FK covering ni PostGIS spatial: `idx_addresses_geo`, `idx_addresses_active`, `idx_addresses_status`, `idx_addresses_created_at`, `idx_profiles_is_active`, `idx_profiles_role`, `idx_streets_active`, `idx_assignments_active`. **Performance advisor: 11 → 7** (preostalih 7 su 2 FK covering na addresses, 1 PostGIS spatial, i 4 nova FK covering na log tabelama koja će se „aktivirati" kad počnu da se koriste).

- **2026-05-12 (Audit log + enum cleanup)**: Migracija `add_status_assignment_logs_and_clean_enum`. **Tri promene:**
  1. **`address_status_log`** tabela — automatski log svake promene `addresses.census_status` (trigger `trg_addresses_status_log` AFTER INSERT/UPDATE OF census_status). Kolone: `address_id`, `old_status`, `new_status`, `changed_by`, `changed_at`. RLS: SELECT-uje ko god može da SELECT-uje parent address (EXISTS pattern). Trigger funkcija je SECURITY DEFINER, EXECUTE revoke-ovan od PUBLIC/anon/authenticated. Backfill: 754 baseline redova (po jedan za svaku postojeću adresu, `old_status=NULL`).
  2. **`assignment_log`** tabela — log za `user_street_assignments` (trigger `trg_user_street_assignments_log` AFTER INSERT/UPDATE/DELETE). Action enum (text+CHECK): `'assigned'`, `'deactivated'`, `'reactivated'`, `'removed'`. RLS: SELECT samo admini (`profiles.role = 'admin'`). Backfill: 22 baseline `'assigned'` redova.
  3. **`census_status_enum` očišćen** — uklonjene neiskorišćene vrednosti `'pending'` i `'refused'`. Ostalo 6: `not_visited`, `visited`, `scheduled`, `students_win`, `sns`, `not_found`. Default promenjen sa `'pending'` na `'not_visited'`. Bilo potrebno DROP/CREATE view-ova (`address_stats`, `addresses_sorted`) i funkcije `get_address_stats_for_user()` jer su sve zavisile od enum-a — vraćeni identično.
  4. **App izmene nisu potrebne** — sve radi automatski preko trigera. UI komponenta za prikaz istorije (npr. „📜 Istorija" tab) je posebna stavka, predlaže se kad bude vreme.

  **Šema sada: 8 živih tabela** (`addresses` 754, `address_status_log` 754, `assignment_log` 22, `user_street_assignments` 22, `profiles` 1, `streets`, `note_templates`, `spatial_ref_sys`).
